...
export interface GanhoFormData {
    descricao: string;
    fonte: string;
    data: string;
    valor: string;
...
interface Props {
    open: boolean;
    onClose: () => void;
    onSubmit: (data: GanhoFormData) => void;
    initialData?: GanhoFormData;
    onDelete?: () => void;
}

const FONTES = ["Trabalho", "Investimentos", "Manutenções", "Freelance", "Outros"];
...
export default function GanhoModal({ open, onClose, onSubmit, initialData, onDelete }: Props) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ animation: "fi .15s ease" }}>
            ...
            <div className="relative bg-white rounded-xl shadow-2xl border border-zinc-200 w-full max-w-lg mx-4" style={{ animation: "si .2s ease" }}>
                <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100">
                    <h3 className="text-base font-semibold text-zinc-900">{editing ? "Editar Ganho" : "Novo Ganho"}</h3>
                ...
                <div className="px-6 py-5 space-y-4">
                    <div className="space-y-1.5">
                        ...
                        <input placeholder="Ex: Salário, Freelance..." value={form.descricao} onChange={e => sf("descricao", e.target.value)} className={inputCls} />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1.5">
                            <label className="block text-sm font-medium text-zinc-700">Fonte de Renda</label>
                            <Combobox items={FONTES} value={form.fonte || null} onValueChange={val => sf("fonte", val ?? "")}>
                    ...
                    <div className={`pt-1 flex gap-3 ${editing ? "" : "flex-col"}`}>
                        ...
                        <button onClick={handleSubmit} className="flex-1 h-10 rounded-md bg-zinc-900 text-white text-sm font-medium hover:bg-zinc-800 active:bg-zinc-700 transition-colors">
                            {editing ? "Salvar" : "Adicionar Ganho"}
                        </button>
                    </div>
                </div>
            </div>
